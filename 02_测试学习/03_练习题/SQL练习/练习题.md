...
orders (
  ...
  currency        VARCHAR(10),   -- 币种（USD / EUR / CNY）
  exchange_rate   DECIMAL(10,4), -- 下单时汇率（兑CNY）
  order_status    VARCHAR(20),   -- 订单状态（CREATED / PAID / SHIPPED / CANCELED）
...
> 查询“已支付（PAID）订单”，并计算每个订单的「人民币金额」，
>  要求只查询“支付时间在 2024-01-01 之后”的订单，
>  最终返回：订单号、用户ID、币种、原始金额、人民币金额（保留 2 位小数）
...
currency,
amount,
round(amount * exchange_rate ,2) AS amount_cny
from 
orders
...
payments (
  ...
  pay_status    VARCHAR(20),   -- SUCCESS / FAIL
  pay_time      DATETIME
)
```
...
> 查询“支付成功（SUCCESS）”的订单，返回： 订单号、用户ID、订单金额、支付时间
...
refunds (
  ...
  refund_status VARCHAR(20)   -- APPLY / SUCCESS / FAIL
)
```

**题目**

> 查询“发生过退款成功”的订单， 并统计每个订单的“退款总金额”

> 只统计 `refund_status = 'SUCCESS'`
>
> 一笔订单可能多次退款
>
> 返回：订单号、退款总金额
...
> 统计每个币种下：
>  已支付（PAID）订单的“订单数量”和“总订单金额”

**要求**

> 按 `currency` 分组
>
> 只统计 `order_status = 'PAID'`
